...
from MicroXO.expando import Expando

from zeroless import Client, Server
# from xo.zmq import xoServer
import killport
from MicroXO.xo_zmq import xoClient
...
class MicroXO(xodal):
	...
	def __init__(self, funcOrNamespace=None, reqPort=None, _xoT_=None, *args, **kwargs):
		...
		_sleep = None if "_sleep" not in kwargs else kwargs.pop("_sleep")
		if _sleep is not None:
			print("SLEEPING", _sleep, type(_sleep))
		runServer = False
		func = None
		...
		if func is not None:
			# print(self._id, "!!!!!!!", func.__name__, type(func))
			MicroXO.index[self._id+"/"+func.__name__] = func
			self.index[self._id+"/"+func.__name__] = func
		...
		if "_services" in kwargs and len(self._services) == 0:
			...
			for service in kwargs["_services"]:
				...
				serviceData = {"client": None, # TODO: reuse clients
								"port": kwargs["_services"][service]["port"],
									}
				if "serviceType" in kwargs["_services"][service] :
					print(f" --- THIS SERVICE {service} HAS A TYPE", kwargs["_services"][service]["serviceType"])
					serviceData["serviceType"] = kwargs["_services"][service]["serviceType"]

				self._services[service] = serviceData
		...
		if self._services is not None:
			...
			if namespace is not None and runServer:
				if namespace in self._services:
					...
					if "serviceType" in self._services[namespace] and self._services[namespace]["serviceType"] != "xoClient":
						useXoClient = False
						runServer = False
		...
		if runServer and reqPort is not None:
			pubPort = MicroXO._pubPort
			if reqPort is None:
				pass
				print(" ::: WTF0 running server without port 0 ::: ",namespace,self._reqPort,self._services)
			...
			def listen(data, *args, **kwargs):
				id, sleep = data["id"], None if "sleep" not in data else data["sleep"]
				if sleep is not None:
					print("########...######",id,type(sleep),sleep)
					sleep(1)
					print(".......")
				...
				for payload in listen_for_request:
					c += 1
					# print("0000000000000000000000000000000",c)
			...
			requests = Thread(target=listen, args=[{"id":self._id, "sleep":_sleep}])
			...
			if len(id.split("/")) > 2:
				...
				if namespace in self._services:
					...
					if self._services[namespace]["client"] is None:
						...
						if "serviceType" in self._services[namespace] and self._services[namespace]["serviceType"] != "xoclient":
							# Defaulting to socketio if not specified
							self._services[namespace]["client"] = socketio.Client()#(_reqPort=self._services[namespace]["port"])
							self._services[namespace]["client"].connect(self._services[namespace]["port"])
						else:
							self._services[namespace]["client"] = xoClient(_reqPort=self._services[namespace]["port"])

					if isinstance(self._services[namespace]["client"], xoClient):
						resp = self._services[namespace]["client"].request(namespace+"/"+target, *payload["args"], ** payload["kwargs"])
					elif isinstance(self._services[namespace]["client"], socketio.Client):
						# resp = self._services[namespace]["client"].call(namespace+"/"+target, *payload["args"], ** payload["kwargs"])
						# resp = self._services[namespace]["client"].call(target, *payload["args"], ** payload["kwargs"])
						try:
							# print(" $$$$$$$ REQUESTING FROM SOCKETIO SERVER $$$$$$$ ", namespace)
							# resp = self._services[namespace]["client"].call(target, [payload["args"], payload["kwargs"]])
							# print('ttttttttartget',target)
							resp = self._services[namespace]["client"].call(target, [payload["args"], payload["kwargs"]], timeout=5)
							# if "timeout" in payload["kwargs"]:

						except:
							print("E x E E E E E E E E E E E E E E E E EEEE\n", namespace,target, "\n", [payload["args"], payload["kwargs"]]								,resp)
							traceback.print_exc()
							resp = {"exception":traceback.format_exc()}
							print()
							# self._services[namespace]["client"].connect(self._services[namespace]["port"])
							# resp = self._services[namespace]["client"].call(
							# 	target, [payload["args"], payload["kwargs"]], timeout=1)
					'''
					'''
					# print(" ::: $$$ GOT REPLY FROM {namespace} / {target} :::", resp)
					# print(resp)
					return resp
