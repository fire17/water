...
class xoClient(Expando):
	...
	def request(self, target="microxo/foo", retry=True, *args, **kwargs):
		...
		try:
			...
			# print(response)
			try:
				if isinstance(response,bytes) and response != b'': #response != None and response != False:
					# print("TTTTTT",type(response))
					response = json.loads(response)
			except:
				print(" ::: WAS UNABLE TO json.loads RESPONSE :::" , response)
				traceback.print_exc()
			...
			self._clientRequest, self._listenReply = self._reqclient.request()
			if retry:
				print("ffffffffffffff, RETRYING", target, args, kwargs)
				return self.request(target=target,retry=False,*args,**kwargs)
			else:
				print("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF",target,self._reqPort)
				traceback.print_exc()
				# return self.request(target=target,retry=False,*args,**kwargs)
			# request, listen_for_reply = client.request()
		finally:
